...
const workExperienceData = [
  ...
  },
  {
    ...
    responsibilities: [
      ...
      "Organizing multi-level labeling schema for toxicit datasets",
      "Ensuring fairness in performance aceoss demographics",
    ],
  },
  {
    role: ["Imaging Intern"],
...
const educationData = [
  {
    ...
    responsibilities: [
      ...
      "Content Moderation &amp Fairness for Group-dependent tasks",
              ...
              <div key={index} className="flex flex-col md:flex-row gap-8">
                <div className="md:w-1/3">
                  ...
                  {Array.isArray(item.company) ? (
                    <p className="text-white/60 mt-1 flex flex-col gap-1">
                      {item.company.map((line, i) => (
                        <span key={i}>{line}</span>
                      ))}
                    </p>
                  ) : (
                    <p className="text-white/60 mt-1">{item.company}</p>
                  )}
                </div>
            ...
            {educationData.map((item, index) => (
              <div key={index} className="flex flex-col md:flex-row gap-8">
                <div className="md:w-1/3">
                  ...
                  <p className="text-white/60 mt-1">{item.degree}</p>
                </div>
                <div className="md:w-2/3">
                  <ul className="flex flex-col gap-4 text-white/60">
                    {item.responsibilities.map((resp, respIndex) => (
